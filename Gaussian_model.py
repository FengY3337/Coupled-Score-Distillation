...
from GaussianSplatting.sh_utils import RGB2SH
from GaussianSplatting.mesh_utils import decimate_mesh, clean_mesh

from GaussianSplatting.mesh import Mesh
...
class GaussianModel:
    ...
    def setup_functions(self):
        def build_covariance_from_scaling_rotation(scaling, scaling_modifier, rotation):
            L = G_utils.build_scaling_rotation(scaling_modifier * scaling, rotation, self.device)
            actual_covariance = L @ L.transpose(1, 2)
            symm = G_utils.strip_symmetric(actual_covariance, self.device)
            return symm
        ...
        self.inverse_opacity_activation = G_utils.inverse_sigmoid
    ...
    def extract_fields(self, resolution=128, num_blocks=16, relax_ratio=1.5):
        ...
        self._xyz = (self._xyz - self.center) * self.scale
        xyzs = self.get_xyz[mask]

        covs = self.covariance_activation(self.get_scaling[mask], 1, self._rotation[mask])

        device = self.get_opacity.device
    ...
    def extract_mesh(self, path, density_thresh=1, resolution=128, decimate_target=1e5):
        ...
        if len(triangles) == 0:
            print("Error: Mesh does not contain any faces.")

        vertices, triangles = clean_mesh(vertices, triangles, remesh=True, remesh_size=0.015)
        if decimate_target > 0 and triangles.shape[0] > decimate_target:
            vertices, triangles = decimate_mesh(vertices, triangles, decimate_target)

        v = torch.from_numpy(vertices.astype(np.float32)).contiguous().to(self.device)
        f = torch.from_numpy(triangles.astype(np.int32)).contiguous().to(self.device)
    ...
    def create_from_pcd(self, pcd: G_utils.BasicPointCloud, spatial_lr_scale: float = 1):
        ...
        opacities = G_utils.inverse_sigmoid(0.1 * torch.ones((fused_point_cloud.shape[0], 1), dtype=torch.float,
                                                             device=self.device))
    ...
    def training_setup(self, opt):
        ...
        l = [
            {'params': [self._xyz], 'lr': opt.lr_position_init * self.spatial_lr_scale, "name": "xyz"},
            {'params': [self._features_dc], 'lr': opt.lr_feature, "name": "f_dc"},
            {'params': [self._features_rest], 'lr': opt.lr_feature / 20.0, "name": "f_rest"},
            {'params': [self._opacity], 'lr': opt.lr_opacity, "name": "opacity"},
            {'params': [self._scaling], 'lr': opt.lr_scaling, "name": "scaling"},
            {'params': [self._rotation], 'lr': opt.lr_rotation, "name": "rotation"}
        ]

        self.optimizer = torch.optim.Adam(l, lr=0.0, eps=1e-15)
        self.xyz_scheduler_args = G_utils.get_expon_lr_func(lr_init=opt.lr_position_init * self.spatial_lr_scale,
                                                            lr_final=opt.lr_position_final * self.spatial_lr_scale,
                                                            lr_delay_mult=opt.lr_position_delay_mult, max_steps=opt.lr_position_max_steps)
    ...
    def reset_opacity(self):
        opacities_new = G_utils.inverse_sigmoid(torch.min(self.get_opacity, torch.ones_like(self.get_opacity) *0.01))
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        rots = G_utils.build_rotation(self._rotation[selected_pts_mask], self.device).repeat(N, 1, 1)
